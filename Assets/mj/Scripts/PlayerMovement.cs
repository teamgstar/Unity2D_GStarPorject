        ...
        switch (m_Status)
        {
            case PlayerStatus.PS_Idle:
                ...
                if (Input.GetKeyDown(KeyCode.Mouse0))
                {
                    ...
                    this.GetComponentInParent<Rigidbody2D>().velocity *= GameManager.g_GameSpeed;
            ...
            case PlayerStatus.PS_Press:
                ...
                if (Input.GetKeyUp(KeyCode.Mouse0))
                {
                    ...
                    this.GetComponentInParent<Rigidbody2D>().velocity = Power;

                    //플레이어 좌우 반전
                    if (this.GetComponentInParent<Rigidbody2D>().velocity.x < 0)
                    {
                        m_Renderer.flipX = true;
                    }
                    else if (this.GetComponentInParent<Rigidbody2D>().velocity.x > 0)
                    {
